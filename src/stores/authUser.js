...
export const useAuthUserStore = defineStore('authUser', () => {
  ...
  async function updateUserImage(file) {
    ...
    const { data, error } = await supabase.storage
      .from('supportlink') // Changed from 'shirlix' to 'supportlink'
    ...
    // If no error set data to userData state with the image_url
      ...
      const { data: imageData } = supabase.storage.from('supportlink').getPublicUrl(data.path) // Changed from 'shirlix' to 'supportlink'
